...
class InfrastructureAgent:
    ...
    def _check_public_exposure(
        ...
        if (
            summary.has_ingress
            and
            summary.public_security_groups > 0
        ):
            ...
            result.add_finding(
                Finding(
                    ...
                    severity=Severity.CRITICAL,
                    message=(
                        "Public ingress combined "
                        "with public security groups"
                    )
                )
            )
            ...
            result.add_recommendation(
                Recommendation(
                    category=Category.SECURITY,
                    message=(
                        "Review ingress exposure and "
                        "restrict security groups"
                    )
                )
            )

    def _check_storage_security(
        self,
        summary,
        result
    ):
