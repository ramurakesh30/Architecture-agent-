...
@router.post("/reports/{report_id}/remediation")
def generate_remediation_plan(
    report_id: str, current_user_id: str = Depends(get_current_user_id)
):

    report = repository.get_report_by_id(report_id, current_user_id)

    if not report:
        raise HTTPException(status_code=404, detail="Report not found")

    remediation_plan = remediation_service.generate_plan(report.report_json)
