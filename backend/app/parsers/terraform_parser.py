...
class TerraformParser:
    def parse(self, tf_content: str):
        ...
        SECRET_PATTERNS = ["password =", "secret =", "api_key =", "access_key ="]
        ...
        for port in DANGEROUS_PORTS:
            if f"from_port = {port}" in tf_content:
                config.open_ingress_ports.append(port)
        if "internal = false" in tf_content:
            config.public_load_balancer = True
        if "associate_public_ip_address = true" in tf_content:
            config.public_ec2_instances = True
        if "map_public_ip_on_launch = true" in tf_content:
            config.public_subnet_usage = True

        if "tags" not in tf_content:
            config.missing_tags = True
        if "aws_db_instance" in tf_content:
            if "storage_encrypted = true" not in tf_content:
